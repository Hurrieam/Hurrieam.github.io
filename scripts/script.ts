$(() => {
    ...
    $(".jumbotron").append(`<a href="${data[0].url}">${data[0].desc} | ${data[0].git_repo}</a>`);
    if (data[0].is_latest) {
    ...
    for (const datum of data) {
        ...
        $(".row").append(`<div class="col-sm-4"><h3>${datum.desc}</h3><a href="${datum.url}">${datum.git_repo}</a></div>`);
        if (datum.is_latest) {
            $(`.row > div:nth-child(${idx - 1}) > h3`).addClass("new");
        }
        idx++;
    }
});
